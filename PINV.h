...
public:
  struct BDCSVD_Mem {
    ...
    Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::ColMajor>
        workspace;
    Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::ColMajor> q0;
   ...
   *
   * //Set up input
   * bd.input.setZero(); //To allow smaller problem sizes to be computed
   * accurately bd.input.block(0, 0, input_matrix.rows(), input_matrix.cols()) =
   *  input_matrix;
   *
   * int32 ret_val = PINV();
   *
   ...
   * allows the functions calling them to not need maximum output buffers every
   * time. The maximum input problem size is currently 2500x17; this can be
   * adjusted inside memory.cpp, where bd is initialized. Any smaller problem
   * size is also allowed, given that the input is zeroed out otherwise.
   */
  void calculate();

  /*
   * Zero out the input so we can set up a new problem.
   */
  void reset();

private:
  /*
   * Perform all heap allocation for the class.
   */
